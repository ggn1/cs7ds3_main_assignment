...
### Are some taste factors better indicators
...
### Visualizing TF-IDF vector reduced to 
### 2 components using T-SNE.
colors <- c("lightgray", "lightgoldenrod1")
palette(colors)
plot(
  wine_reviews$tfidf_tsne_1, wine_reviews$tfidf_tsne_2,
  xlab = "TSNE(TF-IDF Vector) Component 1", 
  ylab = "TSNE(TF-IDF Vector) Component 2", 
  col = factor(wine_reviews$superior_rating),
  pch = 19
)

plot(
  wine_reviews$superior_rating, wine_reviews$tfidf_tsne_2,
  xlab = "Superior Rating", 
  ylab = "TSNE(TF-IDF Vector) Component 2", 
  col = factor(wine_reviews$superior_rating),
  pch = 19
)
...
# OBSERVATIONS:
# * There is a messy, yet visible distinction between superior and
#   non-superior wines. The two TF-IDF compoenents thus, may be 
#   good predictor variables.

### Visualize sweetness, acidity, tannin, alcohol, body.
layout_matrix <- matrix(
  c(1, 2, 3, 4, 5), 
  nrow = 1, byrow = TRUE
)
...
# PREDICTOR VARIABLES = ["price_log10", "Rich", "tannin", "alcohol", "tfidf_tsne_1"x"tfidf_tsne_2"]
...
# NOTE: Here, the interaction term "tfidf_tsne_1" x "tfidf_tsne_2"
#       is considered and not the individual terms because 
#       plotting these terms together v/s individually on a 
#       scatter plot revealed that the clustering observed, only
#       occurred when both terms were present and plotted against
#       each other.

prior_beta_j <- prior(normal(0, 10), class=b)
fit <- brm(
  superior_rating ~ 1 + price_log10 + Rich + tannin + alcohol + 
                    tfidf_tsne_1_norm*tfidf_tsne_2_norm + (1|variety),
